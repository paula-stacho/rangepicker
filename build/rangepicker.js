...
(function ($) {
    ...
    $.fn.rangepicker = function (options) {
        ...
        options = $.extend({
            ...
            defaultStart: (0, _moment2.default)().subtract(6, 'days').format('YYYY-MM-DD'),
            defaultEnd: (0, _moment2.default)().format('YYYY-MM-DD'),
        ...
        var status = {
            lastMonthDisplayed: (0, _moment2.default)(options.lastMonthDisplayed),
            lastSelected: '',
            lastSelectedCompare: '',
            intervalStart: (0, _moment2.default)(new Date(options.defaultStart)).format('YYYY-MM-DD') || null,
            intervalEnd: (0, _moment2.default)(new Date(options.defaultEnd)).format('YYYY-MM-DD') || null,
            compareIntervalStart: (0, _moment2.default)(options.defaultCompareStart).format('YYYY-MM-DD') || null,
            compareIntervalEnd: (0, _moment2.default)(options.defaultCompareEnd).format('YYYY-MM-DD') || null
        };
        ...
        function createCalendarStructure() {
            ...
            content.addClass('rp-popup').click(function (event) {
        ...
        function makeForm() {
            ...
            ///// Date Range
            var dateRangesOptions = [['custom', 'Custom'], ['today', 'Today'], ['yesterday', 'Yesterday'], ['last7Days', 'Last 7 days'], ['last30Days', 'Last 30 days']].reduce(function (list, next) {
                list = list + ('<option value="' + next[0] + '">' + next[1] + '</option>');
                return list;
            }, '');

            var startFormatted = (0, _moment2.default)(status.intervalStart, 'YYYY-MM-DD').format('MMM D, YYYY');
            var endFormatted = (0, _moment2.default)(status.intervalEnd, 'YYYY-MM-DD').format('MMM D, YYYY');
            var dateRanges = $('<div class="rp-daterange-preset-container">\n                    <label for="rp-daterange-preset">Date range:</label>\n                    <select id="rp-daterange-preset" class="rp-daterange-preset">\n                        ' + dateRangesOptions + '\n                    </select>\n                </div>');
            dateFrom = $('<div class="rp-date-input">' + ('<input class="rp-input-mini rp-input-interval focus" type="text" name="range-start" value="' + startFormatted + '">') + '</div> - ');
            dateTo = $('<div class="rp-date-input">' + ('<input class="rp-input-mini rp-input-interval" type="text" name="range-end" value="' + endFormatted + '">') + '</div>');

            ///// Compare date range
            var compareDateRangesOptions = [['mirror', 'Previous'], ['lastYear', 'Previous year'], ['custom', 'Custom']].reduce(function (list, next) {
                list = list + ('<option value="' + next[0] + '">' + next[1] + '</option>');
                return list;
            }, '');
            ...
            var compareDateRanges = $('<div class="rp-comparerange-preset-container">\n                    <label for="rp-comparerange-preset">Compare to period:</label>  \n                </div>');
            compareDateRanges.prepend(showCompare).append(compareRangeOptions);
            compareDateFrom = $('<div class="rp-date-input">' + ('<input class="rp-input-mini rp-input-compare" type="text" name="compare-range-start" value="' + compareStartFormatted + '">') + '</div> - ');
            compareDateTo = $('<div class="rp-date-input">' + ('<input class="rp-input-mini rp-input-compare" type="text" name="compare-range-end" value="' + compareEndFormatted + '">') + '</div>');

            controls = $('<div>');
            var applyBtn = $('<button class="rp-btn rp-applyBtn">Apply</button>');
            var cancelBtn = $('<button class="rp-btn rp-cancelBtn">Cancel</button>');
            controls.append(applyBtn).append(cancelBtn);

            form.append(dateRanges).append(dateFrom).append(dateTo).append(compareDateRanges).append(compareDateFrom).append(compareDateTo).append(controls);
        ...
        function prepareOutputs() {
            ...
            outputs.prepend(outputFrom).append(outputTo);
            ...
            outputsCompare.prepend(outputCompareFrom).append(outputCompareTo);
            if (!status.compareIntervalStart || !status.compareIntervalEnd) {
                outputsCompare.hide();
            }

            self.append(outputs).append(outputsCompare);
        }
        ...
        function prepareWeeks(month) {
            var weeks = [];
            var firstDay = (0, _moment2.default)(month + ' 01', 'YYYY MM DD');
            var lastDay = (0, _moment2.default)(month + ' 01', 'YYYY MM DD').endOf('month');
            var monthNum = firstDay.format('MM');
            var date;
            var todayMoment = (0, _moment2.default)();
            ...
            for (var _i = 0; _i < 6; _i++) {
                ...
                for (var j = 0; j < 7; j++) {
                    ...
                    if (_i === 0 && j < firstDay.day()) {
                        ...
                        var dateFormatted = firstDay.format('YYYY-MM') + '-' + (date > 9 ? date : '0' + date);
                        var selectable = 'rp-selectable';
                        var dateMoment = (0, _moment2.default)(dateFormatted, 'YYYY-MM-DD');

                        if (!options.futureEnabled && dateMoment.isAfter(todayMoment, 'day') || options.maxDateMoment && dateMoment.isAfter(options.maxDateMoment, 'day') || options.minDateMoment && dateMoment.isBefore(options.minDateMoment, 'day')) {
        ...
        function addMonth(month, position) {
            var monthName = (0, _moment2.default)(month + ' 01', 'YYYY MM DD').format('MMMM YYYY');
            var monthNameContainer = $('<span class="rp-month-name">' + monthName + '</span>');

            // add names of days
            var weekHeader = daysOfWeek.map(function (day) {
        ...
        function addBindings() {
            ...
            dateFrom.find('input').on('change', changeInterval).on('focus', function (e) {
                addFocus($(e.currentTarget));
            });
            dateTo.find('input').on('change', changeInterval).on('focus', function (e) {
                addFocus($(e.currentTarget));
            });

            // compare input changes
            compareDateFrom.find('input').on('change', function () {
                calculateCompareCustom('input');
            }).on('focus', function (e) {
                addFocus($(e.currentTarget));
            });
            ...
            compareRangeOptions.on('change', function () {
                calculateCompare();
            });
            ...
            form.find('.rp-daterange-preset-container').on('click', function () {
                addFocus(dateFrom.find('input'));
                handpickInterval();
            });
            form.find('.rp-comparerange-preset-container').on('click', function (event) {
        ...
        function highlightSelection() {
            ...
            months.find('.rp-day').filter(function () {
        ...
        function useDefinedInterval(event) {
            ...
            switch (interval) {
                case 'today':
                    ...
                    break;
                case 'yesterday':
                    var yesterday = (0, _moment2.default)().subtract(1, 'day').format('YYYY-MM-DD');
                    status.intervalStart = status.intervalEnd = yesterday;
                    $('.rp-day[data-date="' + yesterday + '"]').addClass('rp-interval-oneday');
                    break;
                case 'last7Days':
                    start = (0, _moment2.default)().subtract(7, 'day').format('YYYY-MM-DD');
                    end = (0, _moment2.default)().subtract(1, 'day').format('YYYY-MM-DD');
                    ...
                    break;
                case 'last30Days':
                    start = (0, _moment2.default)().subtract(30, 'day').format('YYYY-MM-DD');
                    end = (0, _moment2.default)().subtract(1, 'day').format('YYYY-MM-DD');
                    status.intervalStart = start;
                    status.intervalEnd = end;
                    $('.rp-day[data-date="' + start + '"]').addClass('rp-interval-start');
        ...
        function changeInterval(event) {
            ...
            if (target.attr('name') === 'range-start') {
                status.intervalStart = (0, _moment2.default)(new Date(target.val())).format('YYYY-MM-DD');
            } else if (target.attr('name') === 'range-end') {
                status.intervalEnd = (0, _moment2.default)(new Date(target.val())).format('YYYY-MM-DD');
            }
        ...
        function applyChanges() {
            ...
            if (showCompare.prop('checked')) {
                var startCompareFormatted = (0, _moment2.default)(status.compareIntervalStart, 'YYYY-MM-DD').format('MMM D, YYYY');
                var endCompareFormatted = (0, _moment2.default)(status.compareIntervalEnd, 'YYYY-MM-DD').format('MMM D, YYYY');
        ...
        function resetChanges() {
            status.intervalStart = (0, _moment2.default)(new Date(outputFrom.html())).format('YYYY-MM-DD');
            status.intervalEnd = (0, _moment2.default)(outputTo.html()).format('YYYY-MM-DD');

            status.compareIntervalStart = (0, _moment2.default)(new Date(outputCompareFrom.html())).format('YYYY-MM-DD');
            status.compareIntervalEnd = (0, _moment2.default)(new Date(outputCompareTo.html())).format('YYYY-MM-DD');
        ...
        function setCompareInputs() {
            var startFormatted = (0, _moment2.default)(status.compareIntervalStart, 'YYYY-MM-DD').format('MMM D, YYYY');
            var endFormatted = (0, _moment2.default)(status.compareIntervalEnd, 'YYYY-MM-DD').format('MMM D, YYYY');
        ...
        function calculateInterval(event) {
            if ($(event.currentTarget).hasClass('rp-selectable')) {
                ...
                if (!status.lastSelected || status.lastSelected === 'second') {
                    // selected first date
                    ...
                    if (status.intervalStart < dateClicked) {
                        // selected newer date as second
                        status.intervalEnd = dateClicked;
                    } else {
                        // selected older date as second
                        status.intervalStart = dateClicked;
                    }
        ...
        function calculateCompareMirror() {
            // calculate
            var date = (0, _moment2.default)(status.intervalStart, 'YYYY-MM-DD');
            var length = Math.abs(date.diff(status.intervalEnd, 'days'));
        ...
        function calculateCompareLastYear() {
            // calculate
            status.compareIntervalEnd = (0, _moment2.default)(status.intervalEnd, 'YYYY-MM-DD').subtract('year', 1).format('YYYY-MM-DD');
            status.compareIntervalStart = (0, _moment2.default)(status.intervalStart, 'YYYY-MM-DD').subtract('year', 1).format('YYYY-MM-DD');
        ...
        function calculateCompareCustom(type, event) {
            ...
            if (type === 'input' || $(event.currentTarget).hasClass('rp-selectable')) {
                ...
                if (type === 'input') {
                    // input was changed
                    status.compareIntervalStart = (0, _moment2.default)(new Date(compareDateFrom.find('input').val())).format('YYYY-MM-DD');
                    status.compareIntervalEnd = (0, _moment2.default)(new Date(compareDateTo.find('input').val())).format('YYYY-MM-DD');
                } else if (type === 'click') {
                    var dateClicked = event.target.dataset.date;

                    if (!status.lastSelectedCompare || status.lastSelectedCompare === 'second') {
                        // selected first date
                        ...
                        if (status.compareIntervalStart < dateClicked) {
                            // selected newer date as second
                            status.compareIntervalEnd = dateClicked;
